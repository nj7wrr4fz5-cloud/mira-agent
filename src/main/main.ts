...
const store = new Store<Record<string, ScheduledItem>>();
...
// IPC handlers
ipcMain.handle('get-store-value', (_, k: string) => store.get(k));
ipcMain.handle('set-store-value', (_, k: string, v: any) => { store.set(k, v); return true; });
ipcMain.handle('check-for-updates', () => autoUpdater.checkForUpdates());
ipcMain.handle('install-update', () => autoUpdater.quitAndInstall());

ipcMain.handle('schedule-message', (_, id: string, cronExpr: string, msg: string, chatId: string) => {
  try {
...
ipcMain.handle('cancel-scheduled-message', (_, id: string) => {
...
ipcMain.handle('get-scheduled-messages', () => {
  ...
  for (const key in storeData) {
    if (key.startsWith('scheduled.')) {
      ...
      arr.push({ id: key.replace('scheduled.', ''), ...v });
    }
  }
  return arr;
});
...
app.whenReady().then(() => {
  ...
  for (const key in storeData) {
    if (key.startsWith('scheduled.')) {
      const v = storeData[key] as ScheduledItem;
      const id = key.replace('scheduled.', '');
      try {
        const task = cron.schedule(v.cronExpression, () => mainWindow?.webContents.send('scheduled-message', { id, message: v.message, chatId: v.chatId }));
        scheduledTasks.set(id, task);
